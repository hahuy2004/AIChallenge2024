...
def index2info(lst_idxs):
    ...
    for idx in lst_idxs:
        ...
        if fps_id != 0:
          frame_id_2_ms = frame_id * 1000 / fps_id
        else:
          frame_id_2_ms = 0

        # print(f"f_id: {frame_id}, fps_id: {fps_id}, f_id_2_ms: {frame_id_2_ms}")

        info["lst_keyframe_idxs"].append(frame_id)
        info["lst_keyframe_paths"].append(image_path)
        info["lst_video_idxs"].append(key)
        info["lst_frame_id_2_ms"].append(frame_id_2_ms)
    return info
...
def view_submitted(data):
    ...
    if AnswerDict.get(ques_name, False):
        ...
        lst_video_idxs = result["data"].get("lst_video_idxs", [])
        lst_keyframe_idxs = result["data"].get("lst_keyframe_idxs", [])
        lst_frame_id_2_ms = result["data"].get("lst_frame_id_2_ms",[])
        ...
        for video_idx, keyframe_idx, frame_id_2_ms in zip(lst_video_idxs, lst_keyframe_idxs, lst_frame_id_2_ms):
            print("Ảnh:")
            print("---------------------")
            print("---Text Search---")
            print("{\"answerSets\": [{")
            print("  \"answers\": [")
            print("   {")
            print(f"     \"mediaItemName\": \"{video_idx}\",")
            print(f"     \"start\": {int(frame_id_2_ms)},")
            print(f"     \"end\": {int(frame_id_2_ms)}")
            print("   }]")
            print(" }]")
            print("}")
            ...
            print("  \"answers\": [")
            print("   {")
            print(f"    \"text\": \"...-{video_idx}-{int(frame_id_2_ms)}\"")
            print("   }]")
            print(" }]")
            print("}")

            print()
            print(f"Video_Id: {video_idx} - Time (ms): {int(frame_id_2_ms)} - FrameIdx: {keyframe_idx}")
            print("---------------------")
            print()
