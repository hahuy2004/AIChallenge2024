...
def group_result_by_video_for_url_img(lst_scores, list_idxs, lst_keyframe_idxs, list_image_paths, KeyframesMapper):
  result_dict = dict()
  for i, image_path in enumerate(list_image_paths):
    # data_part, video_id, frame_id = image_path.replace('static\\img\\', '').replace('.webp', '').split('\\')

    data_part, video_id, frame_id = FrameIndexModelInstance.extractInfoFromFileName(image_path)
    ...
    result_dict[key]['lst_keyframe_idxs'].append(int(lst_keyframe_idxs[i]))
    
    result_dict[key]['lst_keyframe_paths'].append(image_path.replace('\\', '/').replace('static/img/', '/data/KeyFrames/'))
    result_dict[key]['lst_idxs'].append(int(list_idxs[i]))
    result_dict[key]['lst_scores'].append(float(lst_scores[i]))

  result = [{'video_id': key, 'video_info': value} for key, value in result_dict.items()]
  result = sorted(result, key= lambda x:x['video_info']['lst_scores'][0], reverse=False)
